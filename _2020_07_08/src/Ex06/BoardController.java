...
public class BoardController {
	...
	public void delete() {
		...
		for(int x= no-1 ;x<i-1;x++) { //i-1을 해줘야 실행갯수를 맞춰주어 오류가 안난다.
			boards[x] = boards[x+1];
			boards[x].no--;
	...
	public void update() {
		...
		if(select.contentEquals("y")) {
			...
			boards[no-1].contents =board1.contents; //시작시 board1에 boards[no-1]값을 넣어주고 시작하고 입력받은후 board[no-1]에 board1을 입력해주어도된다.
