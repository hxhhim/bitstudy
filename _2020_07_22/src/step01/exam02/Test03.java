 ...
 * =>외부에서 인스턴스 변수에 직접 접근하는 대신 이메서드를 호출하여 값을 저장한다.
 * =>메서드 내ㅜㅂ에서 유효한 값을 검증한 후 인스턴스 변수에 저장한다.
 * =>유효하지 않은 값을 넣는것을 방지할수 있다.
 * 
 * 캡슐화(encapsulation)
 * =>중요한(critical) 멤버(변수 또는 메서드)의 직접 접근을 차단하여 
 * 	유효하고 적절할 데이터만 들어가도록 함으로써 데이터의 신뢰성을 높인다.
 * =>유효하지 않은 데이터가 들어가는 것을 방지함으로써 추상화가 무너지지 않게 한다.
 * =>특히 중요한 변수는 셋터와 겟터메서드를 사용하여 간접적으로 값을 넣고 꺼낸게 만든다.
 */
