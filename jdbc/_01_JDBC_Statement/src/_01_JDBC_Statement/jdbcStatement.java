 ...
 * 5.Statement 객체를 생성한다(오라클 Query문 전달용)
 * 6.Query에 결과가 있다면(SELECT문 사용시)
 *   ResultSet 객체를 생성한다.(데이터 목록 저장 객체)
...
public class jdbcStatement {
	...
	static {
		//1)오라클 드라이버 객체를 로딩한다.
		//DriverManager 내부에서 사용할 Oracle
		//드라이버 객체를 생성해서 메모리에 로딩함
		//프로그램에서 직접 접근은 하지 않음으로 클래스 변수에
		//대입하지 않는다.
		try {
